...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        for im in os.listdir(image_foder):
            ...
            cv2.imshow('super_seg0', cv2.cvtColor(
                (rgb.squeeze() * 255).astype(np.uint8), cv2.COLOR_RGB2BGR))
            cv2.imshow('super_seg', cv2.cvtColor(
                r.astype(np.uint8), cv2.COLOR_RGB2BGR))
            cv2.waitKey(0)
